...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.k = k
        self.l = l
        self.gis = nn.ModuleList(
            [self.build_gi(l, m, D, nonlin, n_layers) for _ in range(k)]
        )
    ...
    def build_gi(self, l, m, D, nonlin, n_layers):
        ...
        g = nn.Sequential(nn.Linear(l, D), nonlin)
    ...
    def merge_gis(self):
        ...
        for i in range(k):
            weight0[D * i : D * (i + 1), l * i : l * (i + 1)] = generators[i][
                0
            ].weight.data
            bias0[D * i : D * (i + 1)] = generators[i][0].bias.data
            weight1[m * i : m * (i + 1), D * i : D * (i + 1)] = generators[i][
                2
            ].weight.data
...
def sample_latents(
    ...
    if sample_mode == "random":
        ...
        while z.shape[0] < n:
            ...
            _z = torch.repeat_interleave(torch.rand(_n, l), k, dim=0).reshape(_n, k, l)
            # TODO implement a truly uniform sampling scheme
            # apply random offset sampled uniformly from n-ball
            #   Taken from Sasha's implementation (https://github.com/kotekjedi/object_centric_ood/blob/main/src/sampling_utils.py)
            #   This is "more uniform" than noise from an n-cube w.r.t. to the distance
            #   from the diagonal, but still not perfect
            noise = torch.randn(_n, k + 2, l)
            noise = noise / torch.norm(noise, dim=1, keepdim=True)  # points on n-sphere
            noise = noise[:, :k, :]  # remove two last points
            noise = noise * delta  # scale to radius delta
            _z += noise
            # apply random offset sampled uniformly from n-cube
        ...
        while z.shape[0] < n:
            ...
            mutual_distances = (__z.unsqueeze(2) - __z.unsqueeze(2).transpose(2, 3))[
                :, :, triu_idcs[0], triu_idcs[1]
            ]
        ...
        _z = torch.rand(n, l)
        mask = torch.stack(
            [torch.arange(n), torch.randint(k, (n, 1)).squeeze(dim=1)], dim=1
        ).long()
        z = torch.zeros(n, k, l)
        ...
        _z_diag = torch.repeat_interleave(torch.rand(n_diag, l), k, dim=0).reshape(
            n_diag, k, l
        )
...
class Dataset(torch.utils.data.TensorDataset):
    def __init__(
        self, n: int, k: int, l: int, generator: nn.Module, sample_kwargs: Dict = None
    ):
...
class BatchDataLoader(torch.utils.data.DataLoader):
    def __init__(self, dataset: torch.utils.data.Dataset, batch_size: int) -> None:
        super().__init__(
            ...
            shuffle=None if isinstance(dataset, GenDataset) else True,
        )
...
class NBatchDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(
        self, dataset: torch.utils.data.Dataset, batch_size: int, n_batches: int
    ) -> None:
