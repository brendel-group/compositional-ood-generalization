...
from data import BatchDataLoader, Generator, build_datasets
from models import factory as model_factory
from train_test import Regularizer, comp_contrast, sparse_hess, test, train_epoch
...
def main(cfg):
    ...
    res_dir = f'res/{now:%Y%m%d%H%M%S}_{cfg["name"]}'
    ...
    for gen_cfg in (gen_bar := tqdm(cfg["generators"], position=0)):
        ...
        for data_cfg in (data_bar := tqdm(cfg["datasets"], position=1, leave=False)):
            ...
            for model_cfg in (
                model_bar := tqdm(cfg["models"], position=2, leave=False)
            ):
                model_bar.set_description("Model    ")
                model = model_factory(
                    model_cfg["model"], k, l, m, **model_cfg.get("kwargs", {})
                ).to(dev)
                ...
                train_ldr, test_ldr_id, test_ldr_ood, test_ldr_rand = [
                    BatchDataLoader(dataset, train_cfg.get("bs", 64))
                    for dataset in datasets
                ]

                regularizers = (
                    [
                        Regularizer(**reg_cfg, l=l)
                        for reg_cfg in train_cfg["regularizers"]
                    ]
                    if "regularizers" in train_cfg
                    else None
                )
                ...
                reg_name = (
                    train_cfg["regularizers"][0]["function"]
                    if "regularizers" in train_cfg
                    else None
                )
                reg_weight = (
                    train_cfg["regularizers"][0]["weight"]
                    if "regularizers" in train_cfg
                    else None
                )
                log_dict = {
                    ...
                    "l": l,
                    "m": m,
                    "sampling": data_cfg["display_name"],
                    "n data": n,
                    "sample mode": data_cfg["sample_kwargs"]["sample_mode"],
                    "sample delta": data_cfg["sample_kwargs"].get("delta", None),
                    "model": model_cfg["display_name"],
                    "batch size": train_cfg["bs"],
                    "learning rate": train_cfg["lr"],
                    "regularizer": reg_name,
                    "regularizer weight": reg_weight,
                }

                # print('Train model...')
                for epoch in (
                    epoch_bar := tqdm(
                        range(train_cfg["epochs"]), position=3, leave=False
                    )
                ):
                    ...
                    # train
                    loss = train_epoch(
                        model, train_ldr, optimizer, regularizers=regularizers
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", type=Path, default=Path("cfg.yml"), help="path to config file"
    )
